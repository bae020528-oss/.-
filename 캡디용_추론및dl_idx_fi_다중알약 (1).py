# === Fast Inference API (Multi-pill, Top-1 + item_seq) ===
# YOLO → 최대 4개 알약 bbox 탐지 → 각 알약별 ResNet(1324) Top-1 (dl_idx + item_seq)
# -*- coding: utf-8 -*-
import os
import glob
import json
...
RESNET_1324_PT = os.path.join(DRIVE, "best_model_generalized_fin.pth")
...
LABEL_ROOT = os.path.join(DRIVE, "캡스톤_라벨링_파일")
...
def get_item_seq_from_kcode(kcode: str):
    if not os.path.isdir(LABEL_ROOT):
        return None

    pattern = os.path.join(LABEL_ROOT, "**", f"{kcode}_json", "*.json")
    ...
    try:
        ...
        if "images" in data and len(data["images"]) > 0:
            return data["images"][0].get("item_seq", None)
    except Exception as e:
        print(f"[WARN] item_seq 로드 실패 ({kcode}): {e}")
        return None
...
def infer_pill_image_multi_top1(img_path: str,
    ...
    if not boxes:
        ...
        if preds:
            p = preds[0]
            kcode    = class_idx_to_kcode(p["idx"])
            dl_idx   = kcode_to_dl_idx(kcode)
            item_seq = get_item_seq_from_kcode(kcode)
            results.append({
                "pill_index": 0,
                "bbox": None,
                "idx": p["idx"],
...
if __name__ == "__main__":
    ...
    for r in out:
        print(
            ...
            f"class_idx={r['idx']}, dl_idx={r['dl_idx']}, "
            f"item_seq={r['item_seq']}, prob={r['prob']*100:.2f}%"
        )
    # API에 넘길 땐 out 그대로 json.dumps 해서 쓰면 됨
